...
class PallasLoweringCacheKey:
  """Key used when caching the lowering of an equation."""
  primitive: jax_core.Primitive
  kernel_type: tpu_core.CoreType
  avals_in: tuple[ShapedAbstractValue, ...]
...
class MosaicGridMapping:
  ...
  def __init__(
    ...
    dimension_semantics: tuple[str, ...] = tuple(
    ...
    self.scalar_prefetch_block_shapes = tuple(
        map(_maybe_physicalize_block_shape, scalar_prefetch_avals,
            scalar_prefetch_block_shapes))
    operands_types = []
