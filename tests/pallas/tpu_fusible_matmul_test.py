...
def _fusible_matmul(
  ...
  if impl == KernelImpl.PALLAS_CALL:
    ...
    out = jax.tree.map(jnp.empty_like, z_out_type)
...
class FusibleMatmulTest(jtu.JaxTestCase):
  ...
  def test_matmul(self, dtype, impl):
    ...
    np.testing.assert_allclose(
        jax.jit(fuser.fuse(functools.partial(fusible_matmul, impl=impl)))(x, y),
  ...
  def test_matmul_with_activation(self, dtype, impl):
    ...
    @fuser.fuse
    def matmul_relu(x, y):
      x = fusible_matmul(x, y, impl=impl)
      x = jnp.maximum(x, 0.0)
      return x
  ...
  def test_matmul_reduce_sum(self, impl):
    ...
    def matmul_reduce_sum(x, y):
      x = fusible_matmul(x, y, bm=512, bn=512, impl=impl)
      x = jnp.sum(x, axis=1)
      return x
  ...
  def test_matmul_plus_iota_custom_fusion(self, dtype, impl):
    ...
    def matmul_plus_iota(x, y):
      return (fusible_matmul(x, y, impl=impl).astype(dtype)
              + iota_custom_fusion())
  ...
  def test_matmul_with_bias(self, dtype, impl):
    ...
    x = jax.random.normal(k0, (512, 512), dtype)
    y = jax.random.normal(k1, (512, 512), dtype)
    b = jax.random.normal(k2, (512,), dtype)

    @jax.jit
    @fuser.fuse
    def matmul_bias(x, y, b):
      x = fusible_matmul(x, y, bn=256, impl=impl).astype(dtype)
      x = jnp.maximum(x + jnp.expand_dims(b, 0), 0.0)
      return x

    @jit_no_excess_precision
    def matmul_bias_ref(x, y, b):
      return jax.nn.relu(mm_ref(x, y).astype(dtype) + jnp.expand_dims(b, 0))
  ...
  def test_matmul_with_slice(self, dtype, impl):
    ...
    def matmul_slice(x, y):
      x = fusible_matmul(x, y[1], impl=impl)
      return x
  ...
  def test_matmul_with_dynamic_slice(self, dtype, impl):
    ...
    def matmul_slice(x, y, i):
      x = fusible_matmul(x, y[i], impl=impl)
      return x
  ...
  def test_matmul_with_dynamic_slice_bias(self, dtype, impl):
    ...
    def matmul_slice(x, y, b, i, j):
      x = fusible_matmul(x, y[j], impl=impl).astype(dtype) + b[i]
      return x
  ...
  def test_matmul_with_multi_slice(self, dtype, impl):
    ...
    @fuser.fuse
    def matmul_slice(x, y):
      x = fusible_matmul(x, y[1, 1], impl=impl)
      return x
  ...
  def test_matmul_with_multiple_dynamic_slices(self, dtype, impl):
    ...
    def matmul_slice(x, y, i, j):
      x = fusible_matmul(x, y[i][j], impl=impl)
      return x
  ...
  def test_matmul_with_mixed_slices(self, dtype, impl):
    ...
    @fuser.fuse
    def matmul_slice(x, y, i, j):
      x = fusible_matmul(x, y[2][i, j], impl=impl)
      return x
  ...
  def test_matmul_with_multiple_mixed_slices_and_bias(self, dtype, impl):
    ...
    def matmul_slice(x, y, b, i, j, k):
      x = fusible_matmul(x[k][3], y[2][i, j], impl=impl).astype(dtype)
      return x + b[i, j]
  ...
  def test_matmul_rhs_transpose_no_following_ops(self, dtype, impl):
    ...
    def matmul(impl_fn, x, y):
      # This transpose gets fused in
      z = impl_fn(x, y.T)
      return z

    impl_fused = fuser.fuse(functools.partial(
        matmul, functools.partial(fusible_matmul, bn=256, impl=impl)))
    ref = functools.partial(matmul, mm_ref)

    np.testing.assert_allclose(
        jax.jit(impl_fused)(x, y),
        jax.jit(ref)(x, y),
        atol=5e-5,
    )
  ...
  def test_matmul_rhs_transpose_major_dim(self, dtype, impl):
    ...
    def matmul(impl_fn, x, y):
      ...
      z = impl_fn(x, y.swapaxes(0, 1)[1, 0])
      return z
  ...
  def test_matmul_lhs_transpose(self, dtype, impl):
    ...
    impl_fused = fuser.fuse(functools.partial(
        matmul, functools.partial(fusible_matmul, bm=256, impl=impl)))
    ref = functools.partial(matmul, mm_ref)
  ...
  def test_matmul_out_custom_vjp_bwd(self, dtype, impl):
    ...
    impl_fused = fuser.fuse(functools.partial(
        matmul, functools.partial(fusible_matmul, bn=256, impl=impl)))
    ref = functools.partial(matmul, mm_ref)
    out_dz = jax.jit(impl_fused)(x, y, dz)
...
class ExcessPrecisionTest(jtu.JaxTestCase):
  ...
  def test_matmul_bf16_out(self, impl):
    ...
    impl_fused = fuser.fuse(
        functools.partial(
            matmul,
            functools.partial(fusible_matmul, impl=impl),
        )
    )
  ...
  def test_matmul_bf16_activation(self, impl):
    ...
    impl_fused = fuser.fuse(functools.partial(
        matmul, functools.partial(fusible_matmul, impl=impl)))
    out = jax.jit(impl_fused)(x, y)
  ...
  def test_matmul_f32_out_simple(self, impl):
    ...
    impl = fuser.fuse(
        functools.partial(
            matmul,
            functools.partial(fusible_matmul, bk=256, bn=128, impl=impl),
        )
    )
