...
class VectorSubcoreTest(PallasSCTest):
  ...
  def test_store_to_bitcast(self):
    @self.vector_subcore_kernel(
        out_shape=jax.ShapeDtypeStruct(
            shape=(1, self.num_lanes), dtype=jnp.int32
        ),
    )
    def kernel(o_ref):
...
class ScalarSubcoreTest(PallasSCTest):
  ...
  def test_host_to_hbm_dma(self):
    ...
    def foo(x):
      ...
      y_ref = pl.empty_ref_like(pl.memory_ref(x.shape, x.dtype, pltpu.HBM))
  ...
  def test_hbm_to_host_dma(self):
    ...
    def foo(x):
      ...
      y_ref = pl.empty_ref_like(pl.memory_ref(x.shape, x.dtype, pl.HOST))
