...
class MemRefTest(TestCase):
  ...
  def test_programmatic_serialization_mlir(self):
    ...
    self.assertIn("mosaic_gpu.programmatic_serialization = true", str(module))
...
class TCGen05Test(TestCase, jtu.CudaArchSpecificTest):
  ...
  def test_store_tmem_sparse_meta_layout_raises(self):
    shape = (128, 64)
    tmem_layout = tcgen05.sparse_meta_layout()
    reg_layout = tmem_layout.as_tiled_layout()
    def kernel(ctx, input, output, tmem):
      del ctx
      reg = fa.FragmentedArray.load_untiled(
          input, layout=reg_layout, optimized=False, is_signed=False
      )
      tmem.store(reg)
      # Still calling load on the output, to prevent potential jaxpr DCE.
      tcgen05.commit_tmem()
      tmem.load(reg_layout, is_signed=False).store_untiled(output, optimized=False)

    x = jnp.ones(shape, jnp.uint2)
    with self.assertRaisesRegex(
      NotImplementedError, "Sparse meta layout stores unsupported."
    ):
      mgpu.as_gpu_kernel(
          kernel,
          (1, 1, 1),
  ...
  def test_load_tmem_sparse_meta_layout_raises(self):
    shape = (128, 64)
    tmem_layout = tcgen05.sparse_meta_layout()
    reg_layout = tmem_layout.as_tiled_layout()
    def kernel(ctx, input, output, tmem):
    ...
    x = jnp.ones(shape, jnp.uint2)
    with self.assertRaisesRegex(
      NotImplementedError, "Sparse meta layout loads unsupported."
    ):
      mgpu.as_gpu_kernel(
          kernel,
          (1, 1, 1),
  ...
  @parameterized.parameters(
      ...
      (
        jnp.float16,
        None,
        tcgen05.tmem_half_lane_layout(columns=32, packing=2),
      ),
      (
        jnp.float16,
        None,
        tcgen05.tmem_m64_collective_layout(columns=32, packing=2),
      ),
      (
          jnp.float8_e5m2,
          None,
  ...
  def test_tmem_column_slicing(self, dtype, reg_layout, tmem_layout):
    shape = (tmem_layout.base_tile_shape[0], 64)
  ...
  def test_tmem_column_slicing_custom_layout(self):
    # We test the case where tile number of rows is not a divisor of TMEM_ROWS.
    shape, dtype = (26, 512), jnp.float32
    slicing = (slice(None), slice(256, 512))
    packing = 1
    tmem_layout = tcgen05.TMEMLayout(
      fa.Tiling(((26, packing * 128), (fa.WARP_SIZE * packing,), (packing,))),
      warp_dims=(-3,),
      lane_dims=(-2,),
      vector_dim=-1,
    )
...
class MosaicGpuDialectTCGen05Test(TestCase, jtu.JaxTestCase, jtu.CudaArchSpecificTest):
  ...
  def test_cluster_launch_control(self, dim, cluster):
    # Let's say we have 148 SMs in our gpu. The cluster size is either 1 or 2.
    # We attempt to schedule exactly one more cluster than what fits on the GPU.
    # So either 149 clusters if the cluster size is 1, or 75 if the cluster size
    ...
    assert num_sms % cluster_size == 0

    grid = [1, 1, 1]
    grid[dim] = num_sms // cluster_size + 1
    grid = tuple(g * c for g, c in zip(grid, cluster))

    # Encode the last CTA ID as a single int xxxyyyzzz.
    last_cta_id = (
        (grid[0] - cluster[0]) * 1000000
        + (grid[1] - cluster[1]) * 1000
        + (grid[2] - cluster[2])
    )
    ...
    def single_wg_predicate():
      ...
      return arith.cmpi(arith.CmpIPredicate.eq, wg_indx_in_cta, c(0, wg_type))
    ...
    def body(
      ...
      # Encode x, y, and z into one int xxxyyyzzz in order to compare with the
      # expected result.
      cta_id = z
      cta_id = arith.addi(cta_id, arith.muli(c(1000, i32), y))
      cta_id = arith.addi(cta_id, arith.muli(c(1000000, i32), x))
  ...
  class HypothesisTest(TestCase, jtu.CudaArchSpecificTest):
    ...
    def test_tmem_column_slicing(self):
      ...
      def strategy(draw):
        ...
        initial_tile = (
            draw(hps.sampled_from([32, 64, 128, 256])),
            draw(hps.sampled_from([1, 2, 4, 8, 16])),
        )
        hp.assume(math.prod(initial_tile) >= 128)
